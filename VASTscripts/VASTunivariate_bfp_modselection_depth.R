...
bluepyagg_stn_all <- bluepyagg_stn %>%
  ...
  dplyr::select(Catch_g = meanbluepywt, #use bluepywt for individuals input in example
         ...
         setdepth, #there are 11 stations out of 30538 missing depth
         oisst,
         sstfill
         ) %>%
  na.omit() %>%
...
for(season in mod.season){
  ...
  Q_ikdep <- as.matrix(dat[,c("setdepth")])
  Q_ikdeplennum <- as.matrix(dat[,c("setdepth", "meanpisclen", "npiscsp")])
  Q_ikdeplennumsst <- as.matrix(dat[,c("setdepth","meanpisclen", "npiscsp", "sstfill")])
