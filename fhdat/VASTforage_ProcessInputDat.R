...
write.csv(
  NEAMAPprey,
  here::here("test_SOEpyindex", "orig_NEAMAPprey.csv"),
  row.names = F
)
...
write.csv(
  NEFSCprey,
  here::here("test_SOEpyindex", "orig_NEFSCprey.csv"),
  row.names = F
)
...
write.csv(
  blueprey,
  here::here("test_SOEpyindex", "orig_blueprey.csv"),
  row.names = F
)
...
# save orig nefsc_bluepyagg_stn ----
write.csv(
  nefsc_bluepyagg_stn,
  here::here("test_SOEpyindex", "orig_nefsc_bluepyagg_stn.csv")
)
...
neamap_bluepreyagg_stn <- read_csv(here(
...
neamap_bluepreyagg_stn23 <- read_csv(here(
...
# save orig neamap_bluepreyagg_stn ----
write.csv(
  neamap_bluepreyagg_stn,
  here::here("test_SOEpyindex", "orig_neamap_bluepreyagg_stn.csv")
)
...
# save orig bluepyagg_stn_all -- has NEAMAP temps merged ----
write.csv(
  bluepyagg_stn_all,
  here::here("test_SOEpyindex", "orig_bluepyagg_stn_all.csv")
)
...
for (i in years) {
  ...
  text <- knitr::knit_expand(
    text = "test_{{year}} <- nc_to_raster(nc = name, varname = varname)
...
for (i in years) {
  ...
  text <- knitr::knit_expand(
    text = "sst{{year}} <- raster_to_sstdf(brick = name)
...
for (df in SSTdfs) {
  ...
  yrdietOISST <- do.call(
    'rbind',
    lapply(split(stationsyr, 1:nrow(stationsyr)), function(x) {
      sf::st_join(
        x,
...
saveRDS(
  dietstn_OISST,
  here::here("test_SOEpyindex", "orig_dietstn_OISST_1982_2023.rds")
)
...
saveRDS(
  bluepyagg_stn_all_OISST,
  here("fhdat/bluepyagg_stn_all_OISST_1982-2023.rds")
)
