...
While the second method used in this paper is appealing in its consistent treatment of prey items across the two datasets, the resulting index showed extremely low sensitivity to changing the prey cutoff (Fig. \@ref(fig:preycut)). This suggests that the index is primarily driven by the most commonly identified prey items across the datasets.  

```{r preycut, fig.show='hold', fig.cap="Trend and scale comparison between forage indices using different bluefish prey cut-offs: 10 or more observations by survey and 25 observations combined across surveys (current method).", fig.height=9}
...
As hypothesized, there are differences in both scale and trend for prey indices estimated by VAST for the Gulf of Maine and Georges Bank when excluding assessed prey species (Fig. \@ref(fig:noassessedprey)). There is a difference in scale for the Mid Atlantic, but index trends correspond better in that region.
...
```{r noassessedprey, fig.show='hold', fig.cap="Trend and scale comparison between forage indices using different prey groups: exclude assessed prey (Atlantic herring, silver hake and Atlantic mackerel) and all bluefish prey (current method).", fig.height=9}
...
The number of stomachs by year for each predator show general stability that seem unlikely to explain fluctuations in the index. Table \@ref(tab:nstom) shows number of stomachs by predator included from both the NEFSC and NEAMAP datasets. While there was more sampling overall during the 1990s relative to other portions of the time series, variation in the full Northeast US annual forage index does not correspond to higher or lower numbers of predator stomachs included as input data (Fig. \@ref(fig:nstom)).

```{r nstom, fig.cap="Full Northeast US shelf annual forage index compared with total number of stomachs used as input."}
...
flextable::flextable(stomcountyr) %>%
  flextable::set_caption("Number of stomachs for each predator and year included in analysis, 1985-2021") %>%
  flextable::fontsize(size = 9, part = "all") %>%
  flextable::autofit()
...
All of the models converged with alternative predator inputs. We report the total number of stations included for each model (nstations) and the number of stations lost relative to the full dataset with all predators (stationloss; Table \@ref(tab:altpreds)). The input datasets differ, so comparing AIC across these models is not appropriate, but we report AIC by model for information.
...
```{r compscale1, fig.cap="", fig.height=9}
...
Removing a predator with more stomachs contributing to the input data, white hake, made small changes in the index, but well within the error bars of the original. Changes in trend were most apparent in the Gulf of Maine.
...
```{r comptrend2, fig.cap="", fig.height=9}
...
```{r compscale2, fig.cap="", fig.height=9}
