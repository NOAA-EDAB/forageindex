...
In the following maps of survey station locations by SST match/mismatch for each year, yellow points indicate missing data that would be filled by OISST, and the color range shows how different in situ and OISST temperature is for stations with both values (Figs. \@ref(fig:matchmaps1) - \@ref(fig:matchmaps4)).
...
Index trends estimated by both models are nearly identical (Figs. \@ref(fig:trendcompfall), \@ref(fig:trendcompspring)). Including SST slightly increased index scale relative to the second best fitting model in fall, but had the opposite effect in spring (Figs. \@ref(fig:scalecompfall), \@ref(fig:scalecompspring)). Therefore, including OISST to fill in missing in-situ temperature data seems to have no impact on index trend (as the sst covariate itself has no impact on index trend). 
