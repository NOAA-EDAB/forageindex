...
The full survey dataset included all years from 1973-2021. Because the bluefish assessment begins in 1985, we included only data from 1985-2021 for estimating forage indices. 
...
#### Spatial domain
Models were run using the full Northwest Atlantic spatial grid built into VAST (Fig. \@ref(fig:maps), brown background). Specific strata sets were used from this full model to develop indices matching the spatial extent of different assessment inputs and for broad ecoregions used in ecosystem reporting.
...
Our two-step model selection (1: spatial and spatio-temporal random effects, 2: catchability covariates) was completed using the script `VASTunivariate_bfp_modselection.R` [at this link](https://github.com/sgaichas/bluefishdiet/blob/main/VASTunivariate_bfp_modselection.R). 
...
Seasonal models were run using the script `VASTunivariate_bfp_allsurvs_lencovSST_ALLinoffsplits.R` at [this link](https://github.com/sgaichas/bluefishdiet/blob/main/VASTunivariate_bfp_allsurvs_lencovSST_ALLinoffsplits.R), which contains all stratum definitions. The annual model was run using the script `VASTunivariate_bfp_allsurvsANNUAL_lencovSST_ALLinoffsplits.R` at [this link](https://github.com/sgaichas/bluefishdiet/blob/main/VASTunivariate_bfp_allsurvsANNUAL_lencovSST_ALLinoffsplits.R). 
...
# Acknowledgements
The authors are grateful for the support of the Bluefish Research Track Stock Assessment working group during the development of this index. Working group members included four of the authors (MC, ADW, KD, AST) as well as Karson Cisneros (Mid-Atlantic Fishery Management Council), Sam Truesdell (Massachusetts Department of Marine Fisheries), Jessica Valenti (NMFS NEFSC and Rutgers University), and Samantha Werner (NMFS NEFSC). 
...
flextable::flextable(preycolcode %>% 
  ...
  flextable::set_caption("Prey species names in NEFSC stomach contents and survey data mapped to groups and management categories.") %>%
  #flextable::autofit()
...
compwham <- wham::compare_wham_models(mods, do.plot = F)
...
bays <- data.frame(name = c("Cape Hatteras", "Albermarle Sound", "Chesapeake Bay", "Delaware Bay", "Long Island Sound"," Cape Cod Bay"),
                     Lat = c(-75.5393, -75.9463, -76.105011, -74.947850, -72.9461, -70.3361),
                     Lon = c(35.2480, 36.0787, 37.521374, 39.016490, 41.0748, 41.8451)) %>% 
...
base <- ggplot(data = ecodata::coast) +
  ...
  geom_sf_label(data = nestates, aes(label = NAME),
...
EPUs <- ggplot(data = ecodata::coast) +
  ...
  ggtitle("b. MAB (yellow), GB (orange), GOM (red)")
...
BFsurv <- ggplot(data = ecodata::coast) +
  ...
  ggtitle("c. Bluefish survey (blue) in MABGB (green)")
...
SWaters <- ggplot(data = ecodata::coast) +
  ...
  #coord_sf(xlim = c(-79, -65.5), ylim = c(33, 45)) +
  coord_sf(xlim =c(-78, -65.5), ylim = c(35, 45)) +
  ggtitle("d. State waters (purple), in MABGB (green)")
...
```{r sppprop, fig.cap="Overall proportion of prey groups in stomach contents data (red) compared with survey biomass sampling (black), 1985-2021, all regions and seasons."}
