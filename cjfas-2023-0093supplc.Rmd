---
title: "Assessing small pelagic fish trends in space and time using piscivore stomach contents:  Supplement c, Model results"
...
The VAST model predicts density of forage fish across the entire model domain for each year (Fig. Sc\@ref(fig:springdens)). 

```{r springdens, fig.cap="Yearly maps of VAST model estimated forage fish density for Spring (months 1-6).", out.width='99%'}
...
Spring indices (Fig. Sc\@ref(fig:springall)) for the full spatial domain (AllEPU) through smaller strata were produced.
...
The VAST model predicts density of forage fish across the entire model domain for each year (Fig. Sc\@ref(fig:annualdens)). 

```{r annualdens, fig.cap="Yearly maps of VAST model estimated annual forage fish density (months 1-12).", out.width='99%'}
...
Diagnostics shown for the annual model are as described above for the fall and spring models. 
...
The Annual forage index uses data from all months each year (1-12). The same plots are presented as those above for Fall and Spring indices. Fig. Sc\@ref(fig:annualall) shows all annual forage index time series with standard errors.

```{r annualall, fig.cap="Annual (month 1-12) forage indices scaled to the maximum value for ecoregions Georges Bank (GB), Gulf of Maine (GOM), Mid Atlantic Bight (MAB), and bluefish assessment index areas in state waters within 3 miles of shore (StateWaters) and bottom trawl survey index strata (SurveyBluefish)."}
...
The full results of all models are archived on google drive at [this link](https://drive.google.com/drive/folders/1S-G7QBPq6sUT6dbbf3u9k6k2K46ehDOM?usp=drive_link), with access available upon request.
